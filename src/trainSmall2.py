...
if __name__ == '__main__':
    ...
    if not os.path.isdir('/HaGRID/hagrid_dataset_512'):
        raise FileNotFoundError('Cannot find Training Data')
    trainDir = pathlib.Path('/HaGRID/hagrid_dataset_512').with_suffix('')
    # trainImageCount = len(list(trainDir.glob('*/*.jpg')))
    # Parameters for the Image loader
    batchSize = 32 # Batch size of the Dataset
    # scalingFactor = 8 # scaling factor of the full image
    imgHeight = 512 # desired image height
    imgWidth = 512 # desired image width
    ...
    valSplit = 0.3
    shuffleDataset = True
    trainingDS = tf.keras.utils.image_dataset_from_directory(trainDir, image_size = (imgHeight, imgWidth), validation_split = valSplit, subset = 'training', seed = see, batch_size = batchSize, label_mode = 'categorical', shuffle = shuffleDataset)
    trainingDS.map(lambda x, y: (resizefn(x), y))
    trainingDS.map(lambda x, y: (preprocessinput(x), y))
    # augment data here
    validateDS = tf.keras.utils.image_dataset_from_directory(trainDir, image_size = (imgHeight, imgWidth), validation_split = valSplit, subset = 'validation', seed = see, batch_size = batchSize, label_mode = 'categorical', shuffle = shuffleDataset)
